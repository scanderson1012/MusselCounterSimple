...
def get_app_settings(database_connection: sqlite3.Connection) -> dict[str, int]:
...
def update_app_settings(database_connection: sqlite3.Connection, settings: dict[str, int]) -> dict[str, int]:
    """Persist supported settings and return the validated current values."""
    validated_settings = {
        "fine_tune_min_new_images": _parse_positive_int(settings.get("fine_tune_min_new_images"), 10),
        "fine_tune_num_epochs": _parse_positive_int(settings.get("fine_tune_num_epochs"), 5),
    }
